...
const REQUIRED_FIELDS = [
  'nombreComercio', 'pais', 'provincia', 'ciudad', 'direccion', 'descripcion', 'telefono', 'email'
];
...
async function initializePage() {
  try {
    ...
    setupNavigation();
    createSaveButton();
    checkFormValidation(); // ✅ Validar al cargar
...
async function loadComercioData() {
  try {
    ...
    if (comercioDoc.exists()) {
      ...
      selectedPaymentMethods = comercioData.paymentMethods || [];
...
function fillForm() {
  ...
  if (paisEl) {
    paisEl.value = comercioData.pais || 'Argentina';
  }

  loadProvinciasForCountry(comercioData.pais || 'Argentina');
...
function renderPaymentMethods() {
  ...
  container.innerHTML = methods.map(method => `
    <label class="checkbox-item">
      <input type="checkbox" name="paymentMethods" value="${method}" 
        ${selectedPaymentMethods.includes(method) ? 'checked' : ''}>
  ...
  container.querySelectorAll('input[type="checkbox"]').forEach(cb => {
    cb.addEventListener('change', () => {
      markAsChanged();
      selectedPaymentMethods = Array.from(document.querySelectorAll('input[name="paymentMethods"]:checked'))
        .map(c => c.value);
    });
  });
}
...
function checkFormValidation() {
  ...
  REQUIRED_FIELDS.forEach(fieldName => {
    if (fieldName === 'pais' || fieldName === 'provincia') return;
  ...
  if (JSON.stringify(selectedCategories.sort()) !== JSON.stringify(originalCategories.sort())) {
    hasUnsavedChanges = true;
  }

  // Verificar métodos de pago
  const originalPayment = originalData.paymentMethods || [];
  if (JSON.stringify(selectedPaymentMethods.sort()) !== JSON.stringify(originalPayment.sort())) {
    hasUnsavedChanges = true;
  }

  // Verificar plan
  if (comercioData.plan !== originalData.plan) {
    hasUnsavedChanges = true;
  }
...
function validateRequiredFields() {
  ...
  // Validar que haya al menos un sitio web o red social
...
function setupNavigation() {
  ...
  window.validateCurrentPageData = async () => {
    ...
    // ✅ Bloquear navegación si faltan campos requeridos
    if (!validateRequiredFields()) {
      showToast('Campos incompletos', 'Por favor completa todos los campos requeridos y selecciona al menos una categoría, un método de pago y un sitio web o red social', 'warning');
      return false;
    }
...
async function saveFormData() {
  ...
  try {
    if (!validateRequiredFields()) {
      showToast('Campos incompletos', 'Por favor completa todos los campos requeridos', 'warning');
      return false;
    }
    ...
    updates.paymentMethods = selectedPaymentMethods;
