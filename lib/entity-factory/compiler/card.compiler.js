...
import { CONTEXTUAL_AFFINITY } from '../relations.constants.js';
...
export function compileCard(node) {
  console.log('[card-compiler] node.id:', node.id, '— node.tipo:', node.tipo, '— node.subcategoria:', node.subcategoria);
  const lines = [];

  lines.push('⦓LER:v1.1⦔');
  lines.push(`@node:${node.id}`);

  // Domain — resuelto vía resolveDomain(), fuente de verdad ÚNICA de verdad.
  // node.tipo / node.subcategoria vienen del enrich del index (nivel 1 y 2
  // del árbol de business-vocabulary.json). Si subcategoria falta (entidad
  // vieja sin migrar), resolveDomain cae a su propio fallback por tipo —
  // no lo reimplementamos acá.
  const { domain_tag } = resolveDomain(
    { rubro: { tipo: node.tipo, subcategoria: node.subcategoria } },
    {}
  );
  lines.push(`DOMAIN:${domain_tag}`);
  ...
  // AFF — nota: CONTEXTUAL_AFFINITY sigue indexado por tipo (nivel 1).
  // No es el mismo problema que DOMAIN: la afinidad contextual es correcta
  // a nivel de rubro general (ej. "afinidad con FRR" no depende de si es
  // pizzería o heladería). Se deja así a propósito.
