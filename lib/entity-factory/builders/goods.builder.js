...
export async function buildGoods(comercioRef, context) {
  ...
  groups.forEach((items, key) => {
    if (items.length === 1) {
      ...
      // atributos: excluir url_imagen y tamaño (ruido para el LLM)
      if (p.atributos) {
      ...
      const variantes = items.map(p => {
        ...
        // diferenciador desde atributos explícitos (sin url_imagen ni tamaño)
...
/**
 * Elimina campos de imágenes y tamaño de los atributos —
 * son ruido para el LLM y ya están representados en otros campos.
 */
function cleanAttributes(attrs) {
  const SKIP = ['url_imagen', 'tamaño', 'talla', 'size'];
  return Object.fromEntries(
    Object.entries(attrs).filter(([k]) => !SKIP.includes(k.toLowerCase()))
  );
}
