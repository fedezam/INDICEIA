...
// ─── Interaction model ────────────────────────────────────────
// Fuente de verdad semántica del grafo.
// presence  = dónde ocurre la interacción
// fulfillment = cómo se ejecuta
...
function buildInteraction(data, services) {
  ...
  const presence = {
    physical: data.tieneLocalFisico === true,
    virtual:  hasVirtualCapability(data),
  };

  const fulfillment = {
    delivery: !!entrega.delivery,   // producto viaja al cliente
    pickup:   !!entrega.pickup,     // cliente retira
    salon:    !!entrega.salon,
    takeaway: !!entrega.takeaway,
    on_site:  false,                // prestador va al cliente
  };

  // ── prestadores: leer modalidad desde services ───────────────
  if (data.entityType === 'prestador' && services?.servicios) {
    const modalidades = services.servicios.flatMap(s => s.modalidad || []);
    if (modalidades.includes('a_domicilio')) fulfillment.on_site = true;
...
// capabilities = proyección del interaction model
// mantiene el contrato esperado por router.semantics y card.compiler
function deriveCapabilities(interaction) {
  const { presence, fulfillment } = interaction;

  const caps = {
    presencial: presence.physical,
    delivery:   fulfillment.delivery || fulfillment.on_site, // on_site mapea a delivery para compat
...
function buildOperational(capabilities, interaction, profile, data) {
  ...
  return {
    ...
    on_site:               interaction.fulfillment.on_site, // verdad semántica: prestador va al cliente
  };
}
...
function buildIndexEntry(data, comercioId, goods, services, idxCtx) {
  ...
  // ── modelo de interacción (fuente de verdad) ─────────────────
  ...
  // ── channels: refleja verdad semántica ────────────────────────
  const channels = Object.entries(capabilities)
    .filter(([key, val]) => key !== 'hasAny' && val === true)
    .map(([key]) => {
      if (key === 'delivery' && interaction.fulfillment.on_site) return 'domicilio';
      return key;
    });
  // ────────────────────────────────────────────────────────────
  ...
  return {
    ...
    keywords,
    interaction,   // ← fuente de verdad semántica
    capabilities,  // ← capa compat para router.semantics y card.compiler
    channels,
    ...(schedule ? { schedule } : {}),
    semantic,
    temporal,
