...
function extractKeywords(data, goods, services, semantic = {}) {
  ...
  const sources = [
    ...rawCategories.flatMap(c => c.toLowerCase().split(/\s+/)),
    ...(data.especialidad ? data.especialidad.toLowerCase().split(/\s+/) : []),
    ...(data.descripcion  || '').split(/\s+/),
    ...(semantic.intentMatches || []).flatMap(i => i.split('-')),
    ...(semantic.moods         || []).flatMap(m => m.split('-')),
    ...(semantic.occasions     || []).flatMap(o => o.split('-')),
    ...(semantic.audiences     || []),
  ];
...
function buildInteraction(data, services) {
  const entrega          = data.entrega || {};
  const modalidadTrabajo = data.modalidad_trabajo;
  const hasLocal         = modalidadTrabajo === 'local';

  const presence = {
    physical: data.entityType === 'prestador' ? hasLocal : data.tieneLocalFisico === true,
  ...
  const fulfillment = {
    ...
    salon:    !!entrega.salon,
    takeaway: !!entrega.takeaway,
    on_site:  false,
  };

  if (data.entityType === 'prestador') {
    fulfillment.salon   = hasLocal;
    fulfillment.on_site = modalidadTrabajo === 'a_domicilio';
    if (services?.servicios) {
      const modalidades = services.servicios.flatMap(s => s.modalidad || []);
      if (modalidades.includes('online')) presence.virtual = true;
    }
  }

  return { presence, fulfillment };
}

function deriveCapabilities(interaction) {
  const { presence, fulfillment } = interaction;

  const caps = {
    presencial: presence.physical,
    delivery:   fulfillment.delivery,
    on_site:    fulfillment.on_site,
    pickup:     fulfillment.pickup,
    salon:      fulfillment.salon    || false,
    takeaway:   fulfillment.takeaway || false,
    virtual:    presence.virtual,
  };
...
function compileSchedule(horarios, channelId) {
  ...
  DIAS.forEach(dia => {
    ...
    // Formato profesional: { open: true, turnos: [{ open, close }] }
    if (d.open === true && Array.isArray(d.turnos)) {
      d.turnos.forEach((t, i) => {
        if (t.open && t.close) {
          blocks.push({ from: t.open, to: t.close, label: `turno_${i + 1}`, source: channelId });
        }
      });
    // Formato comercio/prestador: { continuous, morning, afternoon }
...
function buildSchedule(data) {
  const result = {};
  const hasLocal = data.entityType === 'prestador'
    ? data.modalidad_trabajo === 'local'
    : data.tieneLocalFisico === true;

  if (hasLocal) {
    const src = data.horarios || data.horarios_presencial;
    if (src) {
      const s = compileSchedule(src, 'presencial');
      if (s) result.presencial = s;
    }
  }

  if (data.entrega?.delivery && data.horarios_delivery) {
...
function buildTemporal(schedule, profile) {
  ...
  if (openCloseHours.some(b => toHour(b.from) < 8))  patterns.push('abre-temprano');
  if (openCloseHours.some(b => toHour(b.to) >= 20))  patterns.push('abre-tarde');
  if ([...openDays].some(d => FINDE.has(d)))          patterns.push('abre-fines-de-semana');
  if (DIAS.every(d => openDays.has(d)))               patterns.push('abre-todos-los-dias');
...
// ─── Operational ─────────────────────────────────────────────
function buildOperational(capabilities, interaction, profile, data) {
  ...
  return {
    ...
    walkInFriendly,
    urgencyCompatible:     data.atiende_urgencias === true,
    estimatedResponseTime: defaults.estimatedResponseTime || 'medium',
    estimatedDeliveryTime,
    on_site:               interaction.fulfillment.on_site,
  };
}
...
function buildSemantic(data, capabilities, rubroTipo) {
  ...
  if (capabilities.on_site)                          capIntents.push('visita-domicilio', 'coordinar-visita', 'presupuesto');
...
function buildTrust(data, capabilities, schedule, goods, services, idxCtx) {
  const checks = [
    !!data.nombre || !!data.nombreComercio,
    !!data.descripcion,
  ...
  return {
    completenessScore,
    hasSchedule:     !!schedule || !!data.horarios,
...
function buildIndexEntry(data, comercioId, goods, services, idxCtx) {
  ...
  const interaction  = buildInteraction(data, services);
  const capabilities = deriveCapabilities(interaction);
  ...
  const operational = buildOperational(capabilities, interaction, profile, data);
  ...
  const channels = Object.entries(capabilities)
    ...
    .map(([key]) => {
      if (key === 'on_site') return 'domicilio';
      return key;
    });
  ...
  return {
    ...
    subcategoria: idxCtx.rubro.subcategoria,
    tags:         idxCtx.rubro.tags,
    entityType,
    ...
    geo: {
      localidad: {
        ...
        lat:       data.ubicacion?.localidad?.lat    || data.ubicacion?.lat    || data.lat       || null,
        lng:       data.ubicacion?.localidad?.lng    || data.ubicacion?.lng    || data.lng       || null,
      },
    ...
    searchHints,
    trust,
    gateway:   `https://ia.indiceia.dev/c/${slug}`,
    seo:       `https://ia.indiceia.dev/p/${slug}`,
    updatedAt:  new Date().toISOString(),
  };
}
