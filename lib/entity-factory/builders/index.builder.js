...
function buildCapabilities(data) {
  ...
  const caps = {
    ...
    // ✅ fix: delivery existe si hay objeto entrega.delivery (sin requerir .enabled)
...
function buildSchedule(data) {
  ...
  // ✅ fix: Firestore guarda 'horarios' (no 'horarios_presencial')
  if (data.tieneLocalFisico === true && data.horarios) {
    const s = compileSchedule(data.horarios, 'presencial');
    if (s) result.presencial = s;
  }

  // ✅ fix: horarios_presencial como alias por compatibilidad
  if (data.tieneLocalFisico === true && (data.horarios || data.horarios_presencial)) {
    const s = compileSchedule(data.horarios || data.horarios_presencial, 'presencial');
    if (s) result.presencial = s;
  ...
  // delivery: usa horarios_delivery si existe
...
// ─── Operational ─────────────────────────────────────────────
function buildOperational(capabilities, profile, data) {
...
function buildIndexEntry(data, comercioId, goods, services, idxCtx) {
  ...
  const operational  = buildOperational(capabilities, profile, data);
  ...
  // ✅ fix: lat/lng viven en ubicacion.localidad, no en ubicacion raíz
  return {
    id:         slug,
    comercioId,
...
export async function buildIndex(data, comercioId, goods, services) {
  try {
    ...
    // ── debug: diagnóstico de rubro ──────────────────────────
    console.log('[debug-index] comercioId:        ', comercioId);
    console.log('[debug-index] data.rubro:        ', data.rubro);
    console.log('[debug-index] data.tipo_entidad: ', data.tipo_entidad);
    console.log('[debug-index] data.categories:   ', JSON.stringify(data.categories));
    console.log('[debug-index] data.especialidad: ', data.especialidad);
    console.log('[debug-index] idxCtx.rubro:      ', JSON.stringify(idxCtx.rubro));
    // ────────────────────────────────────────────────────────
