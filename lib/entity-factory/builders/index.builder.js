...
function buildIndexEntry(data, comercioId, goods, services, idxCtx) {
  ...
  // ✅ NUEVO: extraer services como señal semántica mínima
  const serviceTags = (services?.servicios || [])
    .map(s => s.n?.toLowerCase())
    .filter(Boolean)
    .slice(0, 5);

  const searchHints = {
    boosts:  [...new Set([...(semantic.bestFor || []), ...serviceTags])],
