...
function buildCapabilities(data, services) {
  ...
  // ── FIX: prestadores leen modalidad desde services ──────────
  if (data.entityType === 'prestador' && services?.servicios) {
    const modalidades = services.servicios.flatMap(s => s.modalidad || []);
    if (modalidades.includes('a_domicilio')) caps.delivery = true;
    if (modalidades.includes('online'))      caps.virtual  = true;
  }
  // ────────────────────────────────────────────────────────────
...
function buildIndexEntry(data, comercioId, goods, services, idxCtx) {
  ...
  const capabilities = buildCapabilities(data, services); // ← services pasado acá
