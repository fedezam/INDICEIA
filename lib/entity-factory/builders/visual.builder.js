...
export async function buildVisual(context, goods, comercioId, services, slug = null) {
  try {
    ...
    if (!isPrestador && items.length === 0) return null;
    ...
    html = html
      ...
      .replace(/\{\{SERVICES\}\}/g,        safify(services?.servicios || []))
    ...
    await db.collection('entidades').doc(comercioId).update({
      ...
      visualPublicUrl:   publicUrl,   // 👈 guardamos la URL pública
      visualGeneratedAt: new Date().toISOString(),
      visualTemplateId:  templateId,
    });

    // 9. Retornar bloque visual
    return {
      available:    !!publicUrl,
      mode:         'html',
      mini_app_url: publicUrl,   // 👈 renombrado
    };
