...
async function compileTemplate(templateId) {
  ...
  // Inyectar CSS inline — el JS lo carga el template via <src>, no lo tocamos
  let compiled = html;
  if (css) {
...
export async function buildVisual(context, goods, comercioId) {
  try {
    const templateId = context.visualTemplateId;
    ...
    // 3. JSON seguro — sin base64, sin complicaciones
    const safeData = JSON.stringify({ nombre, whatsapp, goods: items })
      .replace(/</g, '\\u003c')
      .replace(/>/g, '\\u003e')
      .replace(/&/g, '\\u0026');
