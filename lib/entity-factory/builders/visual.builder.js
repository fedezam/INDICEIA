...
export async function buildVisual(context, comercioRef, comercioId, slug = null) {
  try {
    const templateId = context.templateId;
    if (!templateId) {
      console.warn('[visual-builder] ⚠️ No hay templateId en context — skipping');
      return null;
    }
    ...
    html = html
      ...
      .replace(/\{\{SERVICES\}\}/g,        safify([])) // services no usa el template por ahora
