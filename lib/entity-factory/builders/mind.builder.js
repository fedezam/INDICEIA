...
// ───────────────────────────────────────────────────────────────
//
// ── CANON: implementación mínima (referencia, no contenido) ───
// Implementado como referencia pura: CANON declara DÓNDE vive el
// corpus real de la entidad, sin mover ni duplicar esa data dentro
// del mind. Ver bloque `const CANON` en buildMind — mapea
// entityType → nombre del contrato en entity.json (comercio→goods,
// prestador→services, profesional→professional). soporte no usa
// CANON: ya resuelve la misma pregunta con SABER, inyectando el
// manual como texto plano directamente en el mind.
//
// Responde la pregunta que faltaba, distinta de DOMAIN (¿de qué
// trata mi universo?) y de TRUTH (¿qué reglas rigen ese universo?):
//
//   CANON → ¿qué conjunto de cosas pertenece legítimamente a mi
//            realidad? (lo que puedo reconocer, ofrecer o citar
//            como propio)
//
// Validado en producción (pizzeria-la-esquina, 22/07/2026): tres
// LLMs distintos (MiMo, Grok, ChatGPT) leyeron el mismo mind y
// reconstruyeron correctamente identidad + disponibilidad horaria +
// oferta de la mini app, sin instrucción de comportamiento explícita
// más allá de "cargá y habitá el archivo".
//
// ── Lo que falta para la versión completa (no implementado) ──
// Esta versión mínima NO resuelve:
//   - Materialización para corpus no tabulares (tipo "Platón": textos
//     atribuidos, no products_catalog). Hoy el mapa entityType→contrato
//     asume estructura tipo Firestore array/object; un CANON de texto
//     libre necesitaría su propio mecanismo, más parecido a SABER que
//     a esta referencia.
//   - Si algún día conviene incluir contenido (no solo referencia)
//     dentro del mind para catálogos chicos, o mantener la separación
//     siempre — es una decisión de costo en tokens vs completitud,
//     no resuelta acá.
//   - Retirar las reglas ¬invent dispersas en ORDER_CLOSE/CONTACT_CLOSE
//     ahora que CANON existe. No se tocaron en esta pasada: eran
//     restricciones ya validadas en producción, y quitarlas sin medir
//     el efecto sería un cambio de comportamiento no probado. Quedan
//     como redundancia intencional por ahora, no como error.
// ───────────────────────────────────────────────────────────────
//
...
export function buildMind(data, context, referralCode, visualUrl = '') {
  ...
  // ── CANON (implementación mínima: referencia, no contenido) ──
  // Responde la pregunta que faltaba entre TRUTH ("qué reglas rigen
  // mi mundo") y CAP ("qué puedo hacer"): "¿qué constituye mi
  // realidad?". No se mueve ni se duplica data acá — goods/services/
  // professional siguen viviendo donde ya vivían (contratos aparte
  // en entity.json). CANON solo declara la referencia: el corpus
  // real de esta entidad no es lo que dice TRUTH en abstracto, es
  // el contenido de ese contrato puntual, y nada fuera de él.
  // (Diseño completo — canon materializado por tipo, texto libre
  // tipo "Platón", etc. — sigue pendiente; ver header del archivo.)
  const canonRef = {
    comercio:    'goods',
    prestador:   'services',
  ...
  const CANON = canonRef
    ? `CANON:⟦corpus_real=contrato(${canonRef})∧fuente_unica_de_verdad∧fuera_de(${canonRef})⇒no_existe_para_mi⟧`
    : null;
