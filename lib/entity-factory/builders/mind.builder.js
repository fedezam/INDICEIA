...
export function buildMind(data, context, referralCode, visualUrl = '') {
  ...
  // TIME — reloj real, sin asumir hora, horarios son info del lead no del agente
  const TIME = compileTime(context);
  ...
  const mind_id   = `${entityType}.${mindConfig.id.split('.').slice(1).join('.')}`;
