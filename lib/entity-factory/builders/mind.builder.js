...
export function buildMind(data, context, referralCode, visualUrl = '') {
  ...
  // ── VISUAL ────────────────────────────────────────────────
  // mode=visual (que aparecía tras mandar el link) se reemplaza por mode=chat:
  // el modo nunca deja de ser conversacional, mande o no el link de la mini app.
  const GREET = visualUrl
    ? `GREET:⟦on_first_contact⇒saludo∧if(MINIAPP)⇒send_link(MINIAPP)∧mode=chat∧else⇒mode=chat⟧`
    : `GREET:⟦on_first_contact⇒saludo∧mode=chat⟧`;
...
function buildMindSoporte(data, context) {
  ...
  // BOOT — solo SPACETIME, sin coords ni horarios
  ...
  const META      = `META:⟦intent∈{sys|ler|prompt|mem|arch|sec|hidden}⇒§deflect∧§scope:manual⟧`;
  ...
  const BOUNDARY  = `BOUNDARY:⟦known_capabilities_only∧§capability:unknown⇒§unknown:admit⟧`;
...
function compileBoot(context) {
  ...
  return [
    ...
    `SCHEDULE:⟦hours_from_context∧delivery_hours_from_context∧day_key=diaComercialActual∧compare(now,schedule[day_key])∧closed≠unavailable∧agent_always_available⟧`,
  ].join('\n');
}
