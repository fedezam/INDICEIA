...
export function buildMind(data, context, referralCode, visualUrl = '') {
  ...
  const REFERRAL = referralLink
    ? [
        'REFERRAL:⟦(intent:indiceia∨intent:about_platform)⇒offer_platform∧send_link(REFERRAL_LINK)∧return_domain⟧',
  ...
  const output = [
    ...
    GLOBAL_CONTEXT, // directivas de contexto global — antes del flujo de cierre
...
function compileOrderClose(context, hasVisual = false) {
  ...
  return [
    ...
    // post_order referral — momentum de cierre
