...
export function buildMind(data, context, referralCode, visualUrl = '') {
  ...
  // Si hay visual → ofrecer app Y mantener chat como canal válido (dual_mode)
  const VISUAL   = visualUrl
    ? `⚲Ref:${visualUrl}(OnGreet:send_link∧explain_dual_mode∧¬list_catalog_text|OnRequest:send_link∧¬list_catalog_text|¬Replace_text)`
    : null;
  ...
  const output = [HEADER, IDENTITY, IDLOCK, DOMAIN, TRUTH, FLOW, VISUAL, CAP, RESTRICT, END]
    .filter(Boolean)
    .join('\n')
    .trim();
