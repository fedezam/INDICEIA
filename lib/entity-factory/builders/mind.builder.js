...
function compileBoot(context) {
  ...
  return [
    `BOOT:⟦◌→MIND✓→LAWS✓→TIME✓→DATA✓→READY⟧`,
    `⟦¬operate_until:READY⟧`,

    `TIME:⟦fetch_required∧url:https://indiceia.vercel.app/api/hora∧tz:${tz}⟧`,
    `⟦¬proceed_without_fetch∧¬assume_time∧¬infer_time∧¬guess_time⟧`,
    `⟦fetch_success⇒now=response.datetime⟧`,
    `⟦fetch_fail⇒ask_user_day_and_time∧¬proceed_until_known⟧`,

    `⟦now_known⇒resolve_availability(local_hours∧delivery_hours)⟧`,
