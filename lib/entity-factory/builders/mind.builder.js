...
// de armar un link.
//
// Fix 29/07/2026: se elimina compileSubscription()/SUBSCRIPTION del
// output. Ese mecanismo horneaba el estado de plan (active=false) en
// tiempo de generación (buildEntity()), leyendo data.plan.active en
// ese momento — mismo problema de fondo que tenía el cron de
// vencimiento: si el plan cambiaba de estado DESPUÉS de generarse el
// Blob, este bloque quedaba desactualizado hasta la próxima
// regeneración. Además, solo agregaba una instrucción (¬operate) sin
// remover goods/services/visual/MINIAPP/ORDER_CLOSE del propio mind,
// dependiendo de que el LLM "obedeciera" en vez de ocultar la
// información estructuralmente. El enforcement de plan ahora vive
// enteramente en el proxy (api/entity/[id].js), que lo calcula en
// tiempo real por request vía resolvePlanStatus() + recorta los
// bloques operativos vía stripOperationalBlocks() + agrega el bloque
// ⟦INACTIVE⟧ con el mensaje de huelga definido en mind.config.js
// (inactiveConfig). buildMind()/buildEntity() ya no necesitan saber
// nada sobre el estado del plan — el mind que sale de acá está
// siempre "sano".
//
// Fix 19/08/2026: se agrega compileLeadClose() + closer 'lead'.
// Detectamos que ORDER_CLOSE fuerza el mismo esqueleto transaccional
// (qty+pickup+delivery+direccion) a comercios donde ese modelo no
// existe (autos, maquinaria, industria — el cliente va a ver/probar
// en persona, no hay entrega ni "cantidad" de auto). No se creó un
// entityType nuevo: la entidad sigue siendo 'comercio' con 'goods'
// real (mismo catálogo/precio/schema) — lo único que cambia es el
// modelo de cierre. Por eso la bifurcación NO vive en shape.compiler
// (fijo por entityType) sino en runtime, leyendo
// context.modeloCierre === 'showroom_lead' (seteado en el nuevo step
// de onboarding modelo-cierre.js). compileLeadClose reusa el
// endpoint contact-redirect (generalizado el mismo día para aceptar
// `item` además de `motivo`) en vez de wa-redirect — mismo shape de
// "conectar con una persona", sin lógica de pedido/subtotal.
//
...
//   Auxiliares                → REFERRAL
//
// (El gate "¿Puede operar?" — antes SUBSCRIPTION, primero en la
// lista — ya no vive acá; ver nota de Fix 29/07/2026 arriba.)
//
...
export function buildMind(data, context, referralCode, visualUrl = '') {
  ...
  const CLOSING_COMPILERS = {
    ...
    lead:    (ctx) => compileLeadClose(ctx, resolvedComercioId),
  };
...
// ────────────────────────────────────────────────────────────
// LEAD CLOSE (comercio con modeloCierre=showroom_lead)
// ────────────────────────────────────────────────────────────
//
// Agregado 19/08/2026. Mismo entityType ('comercio') y mismo shape
// que order (goods real, con precio/id/schema) — lo que cambia es
// el modelo de cierre: no hay pedido, hay lead calificado que un
...
//   - Reusa el endpoint contact-redirect (generalizado el mismo día
//     para aceptar `item` además de `motivo`), NO wa-redirect —
//     mismo shape de "conectar con una persona", sin lógica de
//     pedido/subtotal.
//   - El precio se comunica como "consultado", nunca como cerrado
//     (⟦¬prometer_precio_final_cerrado⟧) — el valor real se negocia
//     en persona, el bot no debe sonar como si estuviera cerrando
//     una venta.

function compileLeadClose(context, comercioId) {
  const waNumber = resolveWaNumber(context);
