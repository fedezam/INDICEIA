...
// ── SANITIZE TEXT ─────────────────────────────────────────────
// Corrige espacios dobles y espacios antes de puntuación.
// No toca contenido — solo limpieza tipográfica básica.
...
export function buildContext(data, comercioId, referralCode) {
  ...
  if (hasData(data.aiConfig)) {
    ...
    // Saludo sanitizado — el LLM lo usa textualmente, no puede salir con errores tipográficos
  ...
  // Referral — link listo para usar, el LLM no construye nada
  // El LLM usa este link cuando hace autopublicidad de IndicéIA
  if (hasData(referralCode)) {
    context.referral_link = `https://indiceia.app/r/${referralCode}`;
  }

  // NOTA: templateId y updatedAt se eliminaron del context.
  // templateId → lo necesita buildVisual, se pasa directo desde index.js
  // updatedAt  → metadata del sistema, va en meta del JSON final
