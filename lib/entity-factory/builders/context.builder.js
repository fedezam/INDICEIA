...
function readAiConfig(aiConfig = {}, nombreEntidad = '') {
  ...
  // Saludo generado siempre con el nombre real de la entidad.
  // El texto libre guardado se descarta — es semánticamente peligroso:
  // un LLM priorizará el nombre en el saludo sobre cualquier campo estructurado.
  const saludo = hasData(aiNombre) && hasData(nombreEntidad)
    ? `Hola! soy ${aiNombre}, el asistente de ${nombreEntidad}`
    : aiConfig.identidad?.saludo || aiConfig.aiGreeting || '';
