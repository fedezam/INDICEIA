...
function compileHorarios(horarios, tieneLocalFisico = true) {
  ...
  Object.entries(horarios).forEach(([dia, cfg]) => {
    ...
    if (
      cfg.morning?.enabled &&
      hasData(cfg.morning.open) &&
      hasData(cfg.morning.close)
    ) {
      turnos.push([
        cfg.morning.open,
        cfg.morning.close
    ...
    if (
      cfg.afternoon?.enabled &&
      hasData(cfg.afternoon.open) &&
      hasData(cfg.afternoon.close)
    ) {
      turnos.push([
        cfg.afternoon.open,
        cfg.afternoon.close
...
export function buildContext(data, comercioId, referralCode) {
  ...
  // ── ZONA COBERTURA (solo prestador — zonas geográficas donde trabaja) ─────
  if (data.entityType !== 'profesional') {
    if (
      Array.isArray(data.cobertura) &&
      data.cobertura.length
    ) {
      ubicacion.zona_cobertura = data.cobertura;
      if (!ubicacion.ciudad) {
        ubicacion.ciudad = data.cobertura[0].ciudad;
      }
      if (!ubicacion.provincia) {
        ubicacion.provincia = data.cobertura[0].provincia;
      }
  ...
  ubicacion.local_fisico =
    data.tieneLocalFisico !== false;

  // ── LOCALIDAD (desde data.ubicacion) ──────────────────────
  ...
  const horarios = compileHorarios(
    data.horarios,
    tieneLocal
  );
  ...
  const horariosDelivery = compileHorarios(
    data.horariosDelivery,
    true
  );

  if (horariosDelivery) {
    context.horariosDelivery = horariosDelivery;
  }
  ...
  if (hasData(data.aiConfig)) {
    ...
    if (hasData(ai.nombre)) {
      ia.nombre = ai.nombre;
    }

    if (hasData(ai.saludo)) {
      ia.saludo = sanitizeText(ai.saludo);
    }

    if (hasData(ai.idioma)) {
      ia.idioma = ai.idioma;
    }

    if (hasData(ai.personalidad)) {
      ia.personalidad = ai.personalidad;
    }
    ...
    if (
      ...
      if (hasData(ai.comportamiento.proactividad)) {
        ia.comportamiento.proactividad =
          ai.comportamiento.proactividad;
      }

      if (hasData(ai.comportamiento.formatoRespuestas)) {
        ia.comportamiento.formatoRespuestas =
          ai.comportamiento.formatoRespuestas;
      }
    }
    ...
    if (
      ...
      if (hasData(ai.contingencias.sinPrecio)) {
        ia.contingencias.sinPrecio =
          ai.contingencias.sinPrecio;
      }

      if (hasData(ai.contingencias.sinStock)) {
        ia.contingencias.sinStock =
          ai.contingencias.sinStock;
      }

      if (hasData(ai.contingencias.localCerrado)) {
        ia.contingencias.localCerrado =
          ai.contingencias.localCerrado;
      }
    }
  ...
  if (hasData(referralCode)) {
    context.referral_link =
      `https://indiceia.app/r/${referralCode}`;
  }
