...
import { getSubcategoria } from '../rubro-resolver.js';
...
export function buildContext(data, comercioId, referralCode) {
  ...
  // ── MODELO DE CIERRE ──────────────────────────────────────
  // 'directo' (default implícito si no está seteado) o
  // 'showroom_lead' (autos, maquinaria, inmobiliaria — el cliente
  // ve/prueba en persona, no hay pedido). Seteado en el step de
  // onboarding modelo-cierre.js. Solo tiene efecto real para
  // entityType 'comercio' (ver isShowroomLead en mind.builder.js),
  // pero se copia sin filtrar por entityType acá — el builder de
  // mind es quien decide si aplica.

  if (hasData(data.modeloCierre)) {
    context.modeloCierre = data.modeloCierre;
  }

  // ── DEMO ──────────────────────────────────────────────────
  // Entidad demostradora (ej. PizaBot): no representa un comercio
  // real. No se persiste en el onboarding normal — se setea a mano
  // (Firestore o toggle de super-admin) sobre entidades de prueba.
  // Afecta dos puntos, ambos gateados por este mismo flag, nunca
  // por algo que decida el LLM en runtime:
  //   - mind.builder.js: agrega bloque DEMO (intro/disclaimer).
  //   - closers/order.js: agrega WA_DESTINO al wa_url en vez de
  //     depender solo del whatsapp fijo del comercio.
  //   - api/wa-redirect/[id].js: SOLO ahí se valida en serio —
  //     lee isDemo de Firestore directo (no de este context/LER),
  //     porque es el único punto donde importa que no se pueda
  //     falsear.
  ...
  // ── PRESTADOR ─────────────────────────────────────────────
  // Camino principal: rubro.subcategoria elegido en el rubro-selector
  // se resuelve contra el vocabulario para obtener un nombre legible.
  // Fallback legado: entidades viejas sin rubro.subcategoria, que
  // todavía guardan especialidad como texto libre.

  if (hasData(data.rubro?.subcategoria)) {
    const resuelto = getSubcategoria(data.rubro.subcategoria);
    if (resuelto?.sub?.nombre) {
      context.especialidad = resuelto.sub.nombre;
    }
    // matriculaOficio: dato informativo del rubro-selector (ej. gasista,
    // maestro mayor de obra). NO es lo mismo que context.matricula de
    // más abajo (colegio profesional, entityType 'profesional') — se
    // mantiene en un campo separado para no mezclar shapes distintos
    // (acá es string plano, abajo es {numero, organismo}).
    if (hasData(data.rubro?.matricula)) {
      context.matriculaOficio = data.rubro.matricula;
    }
  } else if (hasData(data.especialidad)) {
  ...
  if (data.entityType === 'profesional') {
    ...
    // Camino principal: rubro.especialidad + rubro.matriculaProf, cargados
    // vía rubro-selector contra business-vocabulary.json (subcategorías
    // SAL-*). Sobreescribe el context.especialidad genérico seteado más
    // arriba en el bloque PRESTADOR con el valor de especialidad clínica
    // específica, que es más preciso que el nombre de la subcategoría sola.
    // Fallback legado: data.matricula plano (entidades viejas pre-migración).
    if (hasData(data.rubro?.especialidad)) {
      context.especialidad = data.rubro.especialidad;
    }

    if (hasData(data.rubro?.matriculaProf?.numero)) {
      context.matricula = {
        numero:    data.rubro.matriculaProf.numero,
        organismo: data.rubro.matriculaProf.organismo || null,
      };
    } else if (hasData(data.matricula?.numero)) {
      context.matricula = {
  ...
  if (hasData(referralCode)) {
    context.referral_link = `https://indiceia.dev/?ref=${referralCode}`;
  }
