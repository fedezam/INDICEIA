...
export function buildContext(data, comercioId, referralCode) {
  ...
  // Formato Firestore: [{ ciudad: "Casilda", provincia: "Santa Fe" }]
  if (Array.isArray(data.cobertura) && data.cobertura.length) {
    ubicacion.cobertura = data.cobertura;
    // ciudad y provincia del primero como fallback para builders que los leen directo
    ...
    // compatibilidad con registros viejos que tenían zona libre
