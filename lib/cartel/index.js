 ...
 * @returns {Promise<HTMLCanvasElement>}
 */
export async function renderPreview({ qrCanvas }) {
  return await renderCartelPreview({ qrCanvas });
}
 ...
 */
export async function exportCartel({ formatId, qrCanvas }) {
  switch (formatId) {
    case 'a4':
      return await exportCartelA4({ qrCanvas });
    default:
