...
export async function exportCartelA4({ qrCanvas }) {
  ...
  // Título — subido de 100px a 130px: más presencia a distancia (vidriera)
  ctx.fillStyle = COLOR.obsidian;
  ctx.font = `700 130px "${FONT.display}", sans-serif`;
  ctx.textBaseline = 'top';
  cursorY = drawMultiline(ctx, titulo, centerX, cursorY, width - PADDING * 2, 152) + 152;

  cursorY += TITLE_GAP;

  // Subtítulo — levemente más grande para acompañar el título nuevo
  ctx.fillStyle = COLOR.moss;
  ctx.font = `500 48px "${FONT.body}", sans-serif`;
  cursorY = drawMultiline(ctx, subtitulo, centerX, cursorY, width - PADDING * 2, 66) + 66;
