...
export function rankCandidates(candidates, parsed, cityIndex, opts = {}) {
  ...
  return candidates.map(node => {
    let score = 0, reasons = [];
    if (parsed.tipo && node.tipo === parsed.tipo && parsed.confidence.tipo >= 0.7) { score += SCORE_WEIGHTS.exactTipo; reasons.push('exactTipo'); }
    if (parsed.normalized && node.keywords?.length) { const m = node.keywords.filter(k => parsed.normalized.includes(k)); if (m.length) { score += SCORE_WEIGHTS.keywordMatch * Math.min(m.length, 3); reasons.push('keywordMatch'); } }
    if (parsed.intents?.length && node.semantic?.intentMatches?.length) { const m = parsed.intents.filter(i => node.semantic.intentMatches.includes(i)); if (m.length) { score += SCORE_WEIGHTS.intentMatch * m.length; reasons.push('intentMatch'); } }
    if (parsed.capabilities?.length) { const caps = Object.keys(node.capabilities||{}).filter(k=>node.capabilities[k]); const m = parsed.capabilities.filter(c=>caps.includes(c)); if(m.length){ score+=SCORE_WEIGHTS.capabilityMatch*m.length; reasons.push('capabilityMatch'); } }
    if (parsed.temporal.openNow) {
      const now=new Date(), day=['domingo','lunes','martes','miercoles','jueves','viernes','sabado'][now.getDay()], hour=now.getHours()+now.getMinutes()/60;
      const s=node.schedule?.presencial?.schedule?.[day]||node.schedule?.delivery?.schedule?.[day];
      if(s&&!s.closed&&(s.blocks||[]).some(b=>hour>=parseTime(b.from)&&hour<=parseTime(b.to))){ score+=SCORE_WEIGHTS.openNow; reasons.push('openNow'); }
    }
    if (parsed.urgency==='high' && node.operational?.urgencyCompatible) { score+=SCORE_WEIGHTS.urgencyCompatible; reasons.push('urgencyCompatible'); }
    ...
    if (parsed.audience && node.semantic?.audiences?.includes(parsed.audience)) { score+=SCORE_WEIGHTS.sameAudience; reasons.push('sameAudience'); }
    if (parsed.occasion && node.semantic?.occasions?.includes(parsed.occasion)) { score+=SCORE_WEIGHTS.sameOccasion; reasons.push('sameOccasion'); }

    return { id: node.id, nombre: node.nombre, tipo: node.tipo, score, reasons: getReasonsForNode(reasons), _node: node };
  }).filter(r => r.score > 0).sort((a,b)=>b.score-a.score).slice(0, maxResults);
}

export function formatResults(ranked, parsed, opts = {}) {
  return {
    parsedQuery: { raw: parsed.raw, tipo: parsed.tipo, intents: parsed.intents, temporal: parsed.temporal, urgency: parsed.urgency, confidence: parsed.confidence },
