...
export function rankCandidates(candidates, parsed, cityIndex, opts = {}) {
  ...
  return candidates.map(node => {
    ...
    // TYPE_PRIORITY: score base normalizado por tipo
    // Desempata cuando varios nodos tienen score semántico similar.
    // SAL(100) → +2.0pts, TRV(45) → +0.9pts — diferencia pequeña pero consistente.
    ...
    // CONTEXTUAL_AFFINITY: boost si el tipo del nodo tiene afinidad
    // con el tipo buscado por el usuario.
    // Ejemplo: usuario busca SAL (médico), nodo es FAR (farmacia) → afinidad 0.92 → +2.76pts
    // Esto permite que tipos relacionados suban aunque no sean exactTipo.
